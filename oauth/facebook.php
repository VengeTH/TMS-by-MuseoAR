...
$facebook_oauth_redirect_uri = "http://dev.aisukuri.mu/oauth/facebook.php";
...
if (isset($_GET["code"]) && !empty($_GET["code"])) {
	...
	if (isset($response["access_token"])) {
		...
		curl_setopt(
			$ch,
			CURLOPT_URL,
			"https://graph.facebook.com/me?fields=id,first_name,last_name,email,picture&access_token=" .
		...
		if ($user == null) {
			...
			$isSuccess = $db->addUser(
				$user_info["first_name"],
				$user_info["last_name"],
				$user_info["email"],
				$user_info["picture"]["data"]["url"],
			);
			if (!$isSuccess) {
